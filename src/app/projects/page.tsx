...
export const metadata = {
  title: "Projects — Crescenzo De Palma",
  description: "Selected projects and work by Crescenzo De Palma.",
};
...
async function getLatestCommits(
  ...
  try {
    const res = await fetch(
      `https://api.github.com/repos/${owner}/${repo}/commits?author=${owner}&per_page=${count}`
    );
    if (!res.ok) return [];
...
export default async function ProjectsListing() {
  ...
  const serialize = (projects: typeof projectsEn): SerializedProject[] =>
    projects.map((p) => ({
      ...
      link: typeof p.meta.link === "string" ? p.meta.link : null,
      linkLabel: typeof p.meta.linkLabel === "string" ? p.meta.linkLabel : null,
